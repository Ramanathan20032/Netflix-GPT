...
// import { movieRatingThunk } from "../../store/thunks/movieRatingThunk";
// import { useState } from "react";
// import RatingModal from "../../components/RatingModal"; // Create this component
...
//     const { ratingLoading } = useSelector((store) => store?.Rating);
...
//         const value = parseFloat(rating); // Convert to float
//         dispatch(movieRatingThunk({ mediaType, movieId: details.id, value }))
//             .then(() => {
//                 setIsRatingModalOpen(false);
//             });
//     }
...
import { resetStatus } from "../../store/slices/movieRatingSlice";
...
const MovieInfoCtaButtons = ({ mediaType }) => {
    ...
    const [isRatingModalOpen, setIsRatingModalOpen] = useState(false);
    const details = mediaType === 'movie' ? movieDetails : tvDetails;
    const { ratingLoading, ratingSuccess, ratingError, ratingMessage } = useSelector((store) => store?.Rating);
    ...
    const handleRatingSubmit = async (rating) => {
        ...
        await dispatch(movieRatingThunk({ mediaType, movieId: details.id, value }));
    }
    ...
    return (
        <>
            ...
            <RatingModal
                ...
                onSubmit={handleRatingSubmit}
                loading={ratingLoading}
                success={ratingSuccess ? "Rated successfully!" : null}
                error={ratingError}
            />
        </>
    )
