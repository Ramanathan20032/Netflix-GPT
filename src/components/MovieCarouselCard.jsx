...
const MovieCarouselCard = ({ movie, mediaType }) => {
    ...
    return (
        <>
            {
                <Link to={`/${routeMediaType}/${movie?.id}`} className="relative rounded-md overflow-hidden transition-all duration-300 hover:scale-105 hover:z-10 cursor-pointer group bg-gray-800 w-full h-full block">
                    {imageUrl ? (
                        <img
                            src={imageUrl}
                            alt={movieTitle}
