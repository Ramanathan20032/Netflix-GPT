...
import { findMediaType, IMAGE_BASE_URL } from "../utils/constants";
...
const VideoTitle = ({ spotLightMovie }) => {
    ...
    const mediaType = findMediaType(spotLightMovie);

    const playTrailer = () => {
        if (!spotLightMovie?.id) return null;
        // console.log("Dispatching trailer thunk with:", { mediaType, movieId: details?.id });
        dispatch(MovieTrailerThunk({ mediaType, movieId: spotLightMovie?.id }));
    }
